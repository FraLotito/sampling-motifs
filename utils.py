...
def motifs_ho_not_full(edges, N, TOT, visited, P=1):
    ...
    for e in edges:
        if len(e) == N - 1 and prob(P):
            nodes = list(e)
            
            for n in nodes:
                for e_i in graph[n]:
                    if len(e_i) == 3 and tuple(nodes) > e_i:
                        continue
                    tmp = list(nodes)
                    tmp.extend(e_i)
                    tmp = list(set(tmp))
...
def motifs_standard(edges, N, TOT, visited, P=1):
    mapping, labeling = generate_motifs(N)
...
def prova(edges, N, TOT, visited, P=1):
    mapping, labeling = generate_motifs(N)
...
def motifs_ho_full(edges, N, TOT, P=1):
    mapping, labeling = generate_motifs(N)
...
def diff_sum2(original, null_models):
    u_null = avg2(null_models)
    res = []
    ...
    for k in original:
        if k not in u_null:
            ...
            tmp = u_null[k]
        res.append((k, (original[k] - tmp) / (original[k] + tmp + 4)))

    for k in u_null:
        if k not in original:
            tmp = u_null[k]
            res.append((k, ((0 - tmp) / (tmp + 4))))
