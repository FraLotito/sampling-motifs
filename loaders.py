...
def load_high_school_timestep(N, A, C=False):
    import networkx as nx
    dataset = "DatasetHigherOrder/High-School_data_2013.csv"
    ...
    prova = list(sorted(list(graph.keys())))
    for i in range(A-1, len(prova), A):
        #print("In {} ci metto ".format(prova[i]))
        try:
            for j in range(A-1):
                graph[prova[i]].extend(graph[prova[i-1-j]])
                #print("{}".format(prova[i-1-j]))
                del graph[prova[i-1-j]]
            #print("FINE")
        except:
...
def load_high_school(N, C=False):
    import networkx as nx
    dataset = "DatasetHigherOrder/High-School_data_2013.csv"
...
def load_primary_school(N, C=False):
    import networkx as nx
    dataset = "DatasetHigherOrder/primaryschool.csv"
...
def load_justice(N, C=False):
    ...
    for _, row in df.iterrows():
        c, n, v, spa = row['caseId'], row['justiceName'], row['vote'], row['justice']
